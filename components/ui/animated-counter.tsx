...
export function Counter({ end, duration = 1.2, fontSize = 24, className = '' }: CounterProps) {
  ...
  useEffect(() => {
    setValue(0)
    if (end <= 0) return
    // Fixed ~60fps: always completes in `duration` seconds regardless of `end` size
    const frameMs = 16
    const totalFrames = Math.ceil((duration * 1000) / frameMs)
    const step = Math.max(1, Math.ceil(end / totalFrames))
    let current = 0
  ...
  return (
    ...
    >
      {end >= 1000 && <Digit place={1000} value={value} slotH={slotH} />}
...
function Digit({ place, value, slotH }: { place: number; value: number; slotH: number }) {
  ...
  const spring = useSpring(rounded, { stiffness: 160, damping: 22 })
